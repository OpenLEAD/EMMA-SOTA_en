...
a provi\-der of burning material; a robot (manipulator) to handle the gun; an
...
the environment. Therefore, some measures must be taken for the execution: no human presence during operation;
environment gases, humidity and temperature must be constantly monitored and controlled; the
robotic manipulator should be sealed; %the wasted particles should be removed afer process (cleaning); 
and emergency system shutdown must be developed.
Table~\ref{tab::hvof} summarizes the project restrictions and specifications.
